...
class ProductDetail {
    ...
    constructor(){
        ...
        this.loadgetProductDetail(responseData);
    ...
    loadgetProductDetail(responseData){
    ...
    loadProductColors(responseData){
        ...
        Object.keys(responseData.pdtColors).forEach(color =>{
            productColors.innerHTML += `<option values = "${color}">${color}</option>`
        })
    }
        ...
        Object.entries(responseData.pdtColors).forEach(entry => {
            if(productColors.value == entry[0]){
                entry[1].forEach(value =>{
                    productSizes.innerHTML += `
                    <input type = "hidden" id = "pdt-dtlDtlId" value = "${value.pdtDtlId}">
